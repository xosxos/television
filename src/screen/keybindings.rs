...
impl Display for DisplayableAction {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let action = match self {
            ...
            DisplayableAction::CopyEntryToClipboard => "Copy entry to clipboard",
...
pub fn build_keybindings_table<'a>(
    ...
    match mode {
        Mode::Channel => build_keybindings_table_for_channel(&keybindings[&mode], colorscheme),
        Mode::RemoteControl => {
            build_keybindings_table_for_channel_selection(&keybindings[&mode], colorscheme)
        }
        Mode::SendToChannel => {
