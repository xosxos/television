...
#[cfg(test)]
#[path = "../../unit_tests/test_config_themes.rs"]
mod tests;
...
static BUILTIN_THEMES: LazyLock<HashMap<&'static str, &'static str>> = LazyLock::new(||
        HashMap::from_iter([
            ...
            ("catppuccin", include_str!("../../config/themes/catppuccin.toml")),
            ("nord-dark", include_str!("../../config/themes/nord-dark.toml")),
            ("solarized-dark", include_str!("../../config/themes/solarized-dark.toml")),
...
impl Theme {
    pub fn from_name(name: &str) -> Self {
        ...
        .unwrap_or_else(|_| Self::from_builtin(name).unwrap_or_else(|_| Self::default()))
    }

    pub fn from_builtin(name: &str) -> Result<Self, Box<dyn std::error::Error>> {
        let theme_content: &str = BUILTIN_THEMES
            .get(name)
            .map_or(BUILTIN_THEMES.get(DEFAULT_THEME).unwrap(), |t| *t);

        Ok(toml::from_str(theme_content)?)
    }
...
impl Default for Theme {
    fn default() -> Self {
        let theme_content = include_str!("../../config/themes/default.toml");
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        Ok(Self {
            ...
            border_fg: Color::from_str(&inner.border_fg).ok_or_else(|| {
                serde::de::Error::custom(format!("invalid color {}", &inner.border_fg))
            })?,
            text_fg: Color::from_str(&inner.text_fg).ok_or_else(|| {
                serde::de::Error::custom(format!("invalid color {}", &inner.text_fg))
            })?,
            dimmed_text_fg: Color::from_str(&inner.dimmed_text_fg).ok_or_else(|| {
                serde::de::Error::custom(format!("invalid color {}", &inner.dimmed_text_fg))
            })?,
            input_text_fg: Color::from_str(&inner.input_text_fg).ok_or_else(|| {
                serde::de::Error::custom(format!("invalid color {}", &inner.input_text_fg))
            })?,
            result_count_fg: Color::from_str(&inner.result_count_fg).ok_or_else(|| {
                serde::de::Error::custom(format!("invalid color {}", &inner.result_count_fg))
            })?,
            result_name_fg: Color::from_str(&inner.result_name_fg).ok_or_else(|| {
                serde::de::Error::custom(format!("invalid color {}", &inner.result_name_fg))
            })?,
            result_line_number_fg: Color::from_str(&inner.result_line_number_fg).ok_or_else(
                || {
            ...
            result_value_fg: Color::from_str(&inner.result_value_fg).ok_or_else(|| {
                serde::de::Error::custom(format!("invalid color {}", &inner.result_value_fg))
            })?,
            selection_bg: Color::from_str(&inner.selection_bg).ok_or_else(|| {
                serde::de::Error::custom(format!("invalid color {}", &inner.selection_bg))
            })?,
            ...
            selection_fg: match inner.selection_fg {
                ...
                None => Color::from_str(&inner.match_fg).ok_or_else(|| {
                    serde::de::Error::custom(format!("invalid color {}", &inner.match_fg))
                })?,
            },
            ...
            preview_title_fg: Color::from_str(&inner.preview_title_fg).ok_or_else(|| {
                serde::de::Error::custom(format!("invalid color {}", &inner.preview_title_fg))
            })?,
            channel_mode_fg: Color::from_str(&inner.channel_mode_fg).ok_or_else(|| {
                serde::de::Error::custom(format!("invalid color {}", &inner.channel_mode_fg))
            })?,
            remote_control_mode_fg: Color::from_str(&inner.remote_control_mode_fg).ok_or_else(
                || {
                    serde::de::Error::custom(format!(
                        "invalid color {}",
            ...
            send_to_channel_mode_fg: Color::from_str(&inner.send_to_channel_mode_fg).ok_or_else(
                || {
                    serde::de::Error::custom(format!(
                        "invalid color {}",
